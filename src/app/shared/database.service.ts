...
export class DatabaseService {
    ...
    public insertBG(data: IBasicSettings) {
        return from(this.database.execSQL('INSERT INTO entries (glucose, dateString) VALUES (?, ?)',
            [+data.bloodGlucose.value, data.bloodGlucose.date.toString()]))
    }

    public insertTreatments(data: IBasicSettings) {
        return from(this.database.execSQL('INSERT INTO treatments (duration, type, basalValue) VALUES (?, ?, ?)',
