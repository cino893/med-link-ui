...
(function () {
  ...
  var taps = -1;
  var R = org.nativescript.TabNavigation.R; // reduces syntax noise, stands for 'android resources'
  var ng = new java.util.Random();
  ...
  android.appwidget.AppWidgetProvider.extend("com.tns.MyWidget", {
    ...
    onUpdate: function (context, appWidgetManager, appWidgetIds) {
      ...
      for (i = 0; i < appWidgetsLen; i++) {
        ...
        console.log(appWidgetsLen.toString());

      }
      clearInterval(this.timer);
      this.timer = setInterval(function () {
        updateWidget(context, appWidgetManager, appWidgetIds, appWidgetIds);
  ...
  function updateWidget(context, appWidgetManager, appWidgetIds, widgetId) {
    ...
    views.setTextViewText(R.id.taps_text, appSettings.getString("BG", text));
